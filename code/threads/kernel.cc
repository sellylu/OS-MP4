...
{
	OpenFile *point = kernel->fileSystem->Open(name);
	if(point == NULL) return -1;
	int result = kernel->fileSystem->PutInTable(point);
	return result;
}
int Kernel::Write(int buf_add, int buf_size, int tid)
{
	char *buffer = &(kernel->machine->mainMemory[buf_add]);
	OpenFile *point = kernel->fileSystem->fileDescriptorTable[tid];
	return kernel->fileSystem->Write(buffer, buf_size ,tid ,point);
}
int Kernel::Read(int buf_add, int buf_size, int tid)
{
	char *buffer = &(kernel->machine->mainMemory[buf_add]);
	OpenFile *point = kernel->fileSystem->fileDescriptorTable[tid];
	return kernel->fileSystem->Read(buffer, buf_size, tid, point);
}
int Kernel::Close(int deleteid)
{
	return kernel->fileSystem->Close(deleteid);
}
